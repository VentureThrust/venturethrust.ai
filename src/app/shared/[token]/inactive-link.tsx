 ...
 * lapsed). We never reveal the reason. Flow:
 *   1. Capture the visitor's email (tracked, so the owner can see who tried).
 *   2. Show a neutral "this link is inactive" screen with two actions:
...
export function InactiveLink({ token }: { token: string; linkId?: string; spaceId?: string }) {
  ...
  async function post(action: string, extra: Record<string, unknown> = {}) {
    const res = await fetch('/api/share-links/contact-owner', {
      ...
      body: JSON.stringify({ token, action, email: email.trim(), ...extra }),
    });
    return res.ok;
  }
